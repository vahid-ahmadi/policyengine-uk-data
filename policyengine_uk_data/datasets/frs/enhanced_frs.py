...
class ExtendedFRS(Dataset):
    def generate(self):
        ...
        original_weights = data["household_weight"][str(self.time_period)]
        for year in range(self.time_period, self.end_year + 1):
            loss_matrix, targets_array = create_target_matrix(
                self.input_frs, year
            )
            new_weights = reweight(original_weights, loss_matrix, targets_array)
...
class EnhancedFRS_2022_23(ExtendedFRS):
