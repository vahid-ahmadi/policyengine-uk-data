...
missing_codes = total_income[
    ~total_income["code"].isin(result_df["constituency_code"])
]["code"].unique()
...
for code in missing_codes:
    for lower, upper in income_bounds:
        new_rows.append(
            {
                "parliamentary constituency 2010": np.nan,  # Set to NaN (missing)
...
result_df_copy = result_df_copy.dropna(subset=["constituency_code"])
# result_df_copy.tail()
    ...
    for zero_constituency in constituencies_with_zero_population:
        try:
            ...
            zero_const_rows = result_df[
                result_df["constituency_code"] == zero_constituency
            ]
            ...
            for _, zero_row in zero_const_rows.iterrows():
                ...
                matching_row = result_df[
                    (result_df["constituency_code"] == nearest_constituency)
                    & (
                        result_df["income_lower_bound"]
